    ...
    def __init__(self, name, root, mode, size, labeled_id_path=None, unlabeled_id_path=None, pseudo_mask_path=None):
        ...
        if mode == 'semi_train' or (self.mode == 'final_train'):
            ...
            if mode == 'test':
                ...
                id_path = 'dataset/splits/%s/ex11/val.txt' % name
            elif mode == 'label':
...
#         print('img path:',os.path.join(self.root, id.split(' ')[0]))
        ...
        if  self.mode == 'label':
            ...
            # mask[mask >=3] = 0
            img2 = img.copy()
            mask_b2 = np.zeros_like(mask)
            mask_b2[mask ==1] = 1
            mask_b2[mask ==2] = 1
            
            
            border = _border_get(mask_b2)
            
            mask_border = np.zeros_like(mask)
            mask_border[mask==1] =1
            mask_border[mask==2] =1
            border = np.squeeze(border) 
            mask_border[border==1] =2
            ...
            # mask = Image.fromarray(mask_array)
            img, mask_cell = normalize(img, mask_cell)
            img, mask_border = normalize(img2, mask_border)
            
            mask_border2 = get_mask_bor(mask)
            mask_border2 = (torch.from_numpy(mask_border2)).to(torch.uint8)

            return img, mask_cell, mask_border2, id
...
#             mask2[mask ==1] = 1
            ...
            # mask[mask >=1] = 1
            img, mask = normalize(img, mask)
            return img, mask,mask_border2, id
...
#             border = np.reshape(border,(border.shape[0],border.shape[1],1))
            mask_border = np.zeros_like(mask)
            
            mask_border[mask==1] =1
            mask_border[mask==2] =1
#             print('mask_border : ', np.unique(mask_border), mask_border.shape)
            border = np.squeeze(border) 
#             print('border', np.unique(border),border.shape)
            mask_border[border==1] =2
            mask_border = np.reshape(mask_border,(mask_border.shape[0],mask_border.shape[1],1))
#             print('mask_border : ', np.unique(mask_border), mask_border.shape)
            ...
            mask_cell = np.reshape(mask_cell,(mask_cell.shape[0],mask_cell.shape[1],1))
...
#             
#             img,mask_cell = transform(image=img,segmentation_maps=mask_cell)#ALL_transform(image=img,segmentation_maps=mask)
            img3, mask3 = img.copy(),mask_cell.copy()
            img2,mask2 =img.copy(),mask_cell.copy()
            mask2 = np.reshape(mask2,(mask2.shape[0],mask2.shape[1],1))
...
#                 img2,mask2 =img.copy(),mask_cell.copy()
                img2,mask2 = ALL_transform(image=img3,segmentation_maps=mask2)
...
#             img2,mask2 =img.copy(),mask_cell.copy()
            ...
            img4, mask_border = normalize(img4, mask_border)
...
#             print('mask_border',mask_border.shape,'mask_cell',mask_cell.shape,'mask2',mask2.shape)
            return img, mask_cell, img2, mask2, img4, mask_border2
