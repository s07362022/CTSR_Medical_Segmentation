...
from dataset.semi_cv import SemiDataset
import random
import numpy as np
import cv2
from copy import deepcopy
from utils import count_params, meanIOU, color_map, AverageMeter, intersectionAndUnion, init_log
from nets.unet_training import CE_Loss, Dice_loss, ContrastiveLoss , Focal_Loss, dice_score_batch
...
pseudo_mask_path = 'outdir/pseudo_masks_ki67/ki67/ex17'
...
def train_model(sup_model, bor_model, optimizer_sup,optimizer_bor, train_loader, val_loader, num_epochs=20, n_classes=3):
    ...
    iters = 0
    cls_weights     = np.array([0.9,1.3,1.0], np.float32)
...
#             loss_bor = criterion_1(outputs_bor, mask_border) + criterion_2(outputs_bor, mask_border)
        ...
        if val_miou > 70:
            ...
            torch.save(best_weights_sup, "/home/u5914116/Harden/SOTA_papper_code/ST-mysel/outdir/models/ki67/ex17/unet_best_sup_weights.pth")
            bor_model.load_state_dict(best_weights_bor)
#             # Optionally, save the best model to disk
            torch.save(best_weights_bor, "/home/u5914116/Harden/SOTA_papper_code/ST-mysel/outdir/models/ki67/ex17/unet_last_border_weights.pth")
...
def evaluate_model(model,model_bor, loader, n_classes=3):
    ...
    tbar = tqdm(loader)
    length = len(loader)
    results = {i: [] for i in range(n_classes)}
    with torch.no_grad():
        for img, mask_cell, id in tbar: #for img, mask_cell, mask_border, id
            images = img.cuda()
...
#             mask_border = mask_border.cuda()
            ...
            label_onehot = torch.nn.functional.one_hot(masks, num_classes=n_classes).permute(0, 3, 1, 2).contiguous()
            pred_onehot = torch.nn.functional.one_hot(pred, num_classes=n_classes).permute(0, 3, 1, 2).contiguous()
    ...
    data_frame = pd.DataFrame(
        data={i: results[i] for i in range(n_classes)},
        index=range(1, length + 1))
    data_frame.to_csv(  './evaluation.csv', index_label='Index')
    result = data_frame.values
    avg_score = np.mean(result, axis=0)
    with open('./performance.txt', 'w') as f:
        f.writelines('metric is {} \n'.format(avg_score[1:]))
    print('AVG DICE Score:', avg_score[1:])
            
    return (total_miou / len(loader))*100 ,  b_miou , (sum(mIOU)/len(mIOU))*100, avg_score[1:]
...
def test_eval(model, loader):
    ...
    with torch.no_grad():
        ...
        val_miou, bor_miou, x_miou , dicescore= evaluate_model(model,None, loader, n_classes=2)
...
sup_model.load_state_dict(torch.load(r'/home/u5914116/Harden/SOTA_papper_code/ST-mysel/outdir/models/ki67/ex18_isic2/unetpp_resnet50_86.77_ISIC_2.pth'))
bor_model = smp.UnetPlusPlus(
                encoder_name = 'resnext50_32x4d',
                encoder_weights="ssl",
                classes = 3,
...
trainset = SemiDataset('ki67', '/work/u5914116/ki67/ex15/', 'train', None, 'dataset/splits/ki67/ex11/labeled.txt')
trainloader = DataLoader(trainset, batch_size=8, shuffle=True,
                             pin_memory=True, num_workers=4, drop_last=True)

valset = SemiDataset('ki67', '/work/u5914116/ki67/ex15/', 'val',None,'dataset/splits/%s/ex18_isic/val.txt')
valloader = DataLoader(valset, batch_size= 1, shuffle=False, pin_memory=True, num_workers=4, drop_last=False)
# valset = SemiDataset('ki67', '/work/u5914116/ki67/ex15/', 'val',None,'dataset/splits/ki67/ex11/val.txt')
# valloader = DataLoader(valset, batch_size= 1, shuffle=False, pin_memory=True, num_workers=4, drop_last=False)

MODE = 'inf'
dataset = SemiDataset('ki67', '/work/u5914116/ki67/ex15/', 'inf', None, None, 'dataset/splits/ki67/ex17/bortest.txt')
...
#                            'dataset/splits/ki67/ex17/labeled.txt', cur_unlabeled_id_path, pseudo_mask_path)
#     trainloader = DataLoader(trainset, batch_size=8, shuffle=True,
...
#     for ix in lab_list:
#         print('\n\n\n================> Total stage 5/5: TEST ex16_quater')
#         sup_model.load_state_dict(torch.load(r'/home/u5914116/Harden/SOTA_papper_code/ST-mysel/outdir/models/ki67/ex17_quater/{}'.format(ix)))
#         print('model: ',ix)
